...
from stacked_bidirectional_rnn_model.params import Params
...
class ClassificationModel(object):
    ...
    def __init__(self,
        ...
        stacked_bidirectional_rnn_layer = self.get_stacked_bidirectional_rnn()(embed_layer)
        # print(stacked_bidirectional_rnn_layer.shape)

        dense_layer = Dense(dense_size,
                            activation="relu",
                            trainable=True)(stacked_bidirectional_rnn_layer)
    ...
    def get_stacked_bidirectional_rnn(self):
        model = Sequential()
        # first layer
        model.add(Bidirectional(SimpleRNN(Params.rnn_units,
                                          return_sequences=True,
                                          return_state=False,
                                          trainable=True,
                                          input_shape=(Params.max_sent_size, Params.embed_size))))
        # middle layers
        for i in range(Params.n_stacked_rnn - 2):
            model.add(Bidirectional(SimpleRNN(Params.rnn_units,
                                              return_sequences=True,
                                              return_state=False,
                                              trainable=True)))
