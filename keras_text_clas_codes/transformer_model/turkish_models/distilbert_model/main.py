...
from transformer_model.turkish_models.distilbert_model.preprocess import *
from transformer_model.turkish_models.distilbert_model.mydataloader import MyDataloader
from transformer_model.turkish_models.distilbert_model.mymodel import MyModel
from transformer_model.turkish_models.distilbert_model.params import Params
...
class Main(object):
    ...
    def run_train(dataloader):
        Params.pipeline_model.train()  # set train mode
        epoch_loss = []
        predicted_labels = []
        y_labels = []
        for id, (X, y) in enumerate(dataloader):  # batch
            ...
            encoding = Params.tokenizer(list(X),
                                        ...
                                        padding=True,
                                        truncation=True,
                                        max_length=Params.sequence_max_length,
            ...
            outputs = Params.pipeline_model(input_ids, attention_mask)
            #print(outputs)
    ...
    def run_test(dataloader):
        Params.pipeline_model.eval()  # set eval mode
        epoch_loss = []
        predicted_labels = []
        y_labels = []
        for id, (X, y) in enumerate(dataloader):  # batch
            ...
            outputs = Params.pipeline_model(input_ids, attention_mask)  # bert
            # print(outputs)
            loss = Params.criterion(outputs, y.long())  # y: long type
            epoch_loss.append(loss.item())  # save batch loss
    ...
    def run_train_test(train_dataloader, test_dataloader):
        Params.pipeline_model = MyModel(model=Params.model,
        ...
        # push model to gpu
        if Params.use_cuda:
            Params.pipeline_model = Params.pipeline_model.cuda()

        pytorch_total_params = sum(p.numel() for p in Params.pipeline_model.parameters())
        print("pytorch_total_params: ", pytorch_total_params)

        Params.optimizer = torch.optim.Adam(Params.pipeline_model.parameters(), lr=Params.lr)
